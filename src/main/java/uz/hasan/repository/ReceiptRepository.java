...
public interface ReceiptRepository extends JpaRepository<Receipt, Long> {
    ...
    @Query("select distinct receipt from Receipt receipt left join fetch receipt.cars left join fetch receipt.addresses")
    List<Receipt> findAllWithEagerRelationships();

    @Query("select receipt from Receipt receipt left join fetch receipt.cars left join fetch receipt.addresses where receipt.id =:id")
